...
def lasso_cv_plus_model_selection(X0, y0, k, group_labels, rand_added_flag):
    ...
    lasso_cv = linear_model.LassoCV(
        alphas=alphas, random_state=0, max_iter=1000, selection="random", n_jobs=k
    )
...
def SVR_cv_plus_model_selection(X0, y0, k, group_labels, rand_added_flag):
    ...
    if rand_added_flag:
        scores_rand = cross_val_score(
            svr_gs, X, y0.sample(frac=1), groups=group_labels, cv=split_obj, n_jobs=n_j
        )
    else:
        scores_rand = 0
    return scores, scores_rand
