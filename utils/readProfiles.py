...
def read_replicate_level_profiles(
    ...
    cp_data_repLevel = pd.read_csv(
        dataDir + "/CellPainting/replicate_level_cp_" + profileType + ".csv.gz"
    )
...
def read_treatment_level_profiles(
    dataset_rootDir, dataset, profileType, filter_perts, per_plate_normalized_flag
):
    ...
    meta_cp = (
        cp_data_repLevel[[labelCol] + meta_dict[dataset][0]]
        .drop_duplicates()
        .reset_index(drop=True)
    )
    meta_l1k = (
        l1k_data_repLevel[[labelCol] + meta_dict[dataset][1]]
        .drop_duplicates()
        .reset_index(drop=True)
    )
...
def generate_random_match_of_replicate_pairs(cp_data_repLevel, l1k_data_repLevel, nRep):
    ...
    mergedProfiles_repLevel = pd.merge(
        cp_data_n_repLevel, l1k_data_n_repLevel, how="inner", on=[labelCol]
    )
...
def read_paired_replicate_level_profiles(
    dataset_rootDir, dataset, profileType, nRep, filter_perts, per_plate_normalized_flag
):
